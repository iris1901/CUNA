...
class ReadEmbed(nn.Module):
    ...
    def forward(self, batch_base_seq):
        return self.read_emb(batch_base_seq)

# Encodes the input base sequence using a specified embedding strategy (one-hot or learnable).
class SeqEmbed(nn.Module):
    ...
    def forward(self, batch_base_seq):
        return self.seq_emb(batch_base_seq)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x_pos = self.pe.repeat(x.size(0), 1, 1)
...
class BiLSTM(nn.Module):
    def __init__(self, model_dims, num_layers, dim_feedforward, num_fc, embedding_dim, embedding_type, fc_type):
        ...
        self.emb=SeqEmbed(embedding_dim, embedding_type)
...
class TransformerModel(nn.Module):
    def __init__(self, model_dims, num_layers, dim_feedforward, num_fc, embedding_dim, embedding_type, pe_dim, nhead, pe_type, fc_type):
