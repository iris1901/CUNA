...
# Segments the raw signal using the move table and normalizes it (MAD or Z-score).
# Returns a matrix of events and indexing structures used for full-signal models.
@jit(nopython=True)
...
# Extracts sumarized features (mean, std, quantiles, MAD, length) from raw signal segments defined by the move table, for use in non-full-signal models
@jit(nopython=True)
...
# Identifies candidate motif positions within the read sequence and maps them to the aligned reference positions using aligned pairs.
...
# Writes model predictions to a BAM file with MM/ML tags, and saves tab-delimited files with per-read and per-site modification scores and statistics.
...
# Main processing function: extracts features from the signal, calls the model, generates predictions, and queues the results for output writing.
...
# Reads POD5 files and matches them with BAM reads. Extracts raw signal and alignment data, then queues the data in batches for downstream processing.
...
# Entry point for the detection pipeline. Manages multiprocessing of input loading, signal processing, and output writing across all threads.
