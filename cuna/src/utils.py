...
def revcomp(s):
 ...
 # Loads a trained model (BiLSTM or Transformer) and its configuration from the given paths
def get_model(params): 
    try:
        model_config_path, model_path = params['model'].split(',')
    except ValueError:
        print(f"--model must hace the format: config_path,checkpoint_path", flush=True)
        sys.exit(2)
    ...
    if model_config['model_type'] == 'bilstm':
        ...
        print(f"Unrecognized model: {model_config['model_type']}", flush=True)
        sys.exit(2)

# Yields batches of input features and base sequences for model inference
...
# Computes aligned read-reference base pairs from CIGAR string and reference start
@jit(nopython=True)
...
# Validates and expands a user-specified motif and its target positions
...
# Aggregates per read modification scores into per site modification counts and writes output file
...
# Starts per site prediction output generation based on per reaqd predictions
