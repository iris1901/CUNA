...
def train(training_dataset, validation_dataset, validation_type, validation_fraction, model_config, epochs, prefix, retrain, batch_size, args_str, seed, args):
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        for j in range(epochs):
            ...
            train_gen = utils.generate_batches_mixed(training_dataset, validation_type, validation_fraction, data_type="train", batch_size=batch_size)
            for batch in train_gen:
                ...
                optimizer.step()
                utils.get_metrics(metrics_train, 'Normal', batch_y, score, loss)

            net.eval()
            with torch.no_grad():
                if validation_type == 'split':
                    test_gen = utils.generate_batches(list(itertools.chain.from_iterable(training_dataset)), validation_type, validation_fraction, data_type="test", batch_size=batch_size)
                else:
                    test_gen = utils.generate_batches(validation_dataset, validation_type, validation_fraction, data_type="test", batch_size=batch_size)
                ...
                for batch in test_gen:
                    ...
                    score = net(batch_x, batch_base_seq)
                    loss = F.binary_cross_entropy_with_logits(score, batch_y, pos_weight=pos_weight)
                    utils.get_metrics(metrics_test, 'Normal', batch_y, score, loss)

            train_str, train_vals_dict = utils.get_stats(metrics_train, 'Training', return_dict=True)
            test_str, test_vals_dict = utils.get_stats(metrics_test, 'Testing', return_dict=True)
            ...
            train_gen_eval = utils.generate_batches(list(itertools.chain.from_iterable(training_dataset)), validation_type, validation_fraction, data_type="train", batch_size=batch_size)
            test_gen_eval = utils.generate_batches(validation_dataset, validation_type, validation_fraction, data_type="test", batch_size=batch_size) if validation_type == "dataset" else \
                            utils.generate_batches(list(itertools.chain.from_iterable(training_dataset)), validation_type, validation_fraction, data_type="test", batch_size=batch_size)
...
def main(args):
    ...
    mixed_training_dataset = utils.get_files(args.mixed_training_dataset)
    validation_dataset = utils.get_files(args.validation_dataset)
    validation_type = args.validation_type
    validation_fraction = args.validation_fraction

    valid_data, window, norm_type, strides_per_base, model_depth, full_signal = utils.check_training_files(
