...
# Collects all `.npz` files from folders or path lists, and shuffles them randomly.
...
# Loads features, sequences, and labels from a `.npz` file.
# Optionally performs train/test split and can return positions if `get_pos=True`.
def read_from_file(file_name, validation_type, validation_fraction, data_type, get_pos=False):
    ...
    if get_pos:
        ...
        return features, base_seq, labels, pos_data

    if validation_type == 'split':
        features_train, features_test, base_seq_train, base_seq_test, labels_train, labels_test = \
            train_test_split(features, base_seq, labels, test_size=validation_fraction,
                             random_state=42, stratify=labels)
        if data_type == 'train':
            return features_train, base_seq_train, labels_train
        else:
            return features_test, base_seq_test, labels_test

    else:
        return features, base_seq, labels

# Checks that all `.npz` files have consistent key parameters (window, normalization, etc.).
# Returns `False` if inconsistencies are found.
def check_training_files(mixed_training_dataset, validation_dataset):
    all_files = list(itertools.chain.from_iterable([mixed_training_dataset, validation_dataset]))

    norm_type = [str(np.load(file)['norm_type']) for file in all_files]
    window = [int(np.load(file)['window']) for file in all_files]
    strides_per_base = [int(np.load(file)['strides_per_base']) for file in all_files]
    model_depth = [int(np.load(file)['model_depth']) for file in all_files]
    full_signal = np.unique([np.load(file)['full_signal'] for file in all_files])
...
# Generates data batches from `.npz` files based on validation type and mode (training or test).
def generate_batches(files, validation_type, validation_fraction, data_type, batch_size=512):
    ...
    while counter < len(files):
        ...
        for local_index in range(0, labels.shape[0], batch_size):
            ...
            yield batch_x, batch_base_seq, batch_y

        if counter % print_freq == 0:
            print('.', end='', flush=True)

# Generates balanced batches from multiple mixed datasets.
# Adjusts batch size proportionally to the size of each dataset group.
def generate_batches_mixed(data_file_list, validation_type, validation_fraction, data_type, batch_size=128): 
    ...
    generators = [
        generate_batches(group_files, validation_type, validation_fraction, data_type, batch_size=group_batch_size)
    ...
    for multi_batch_data in zip(*generators):
        ...
        batch_y = torch.vstack([d[2] for d in multi_batch_data if len(d[2]) > 0])
        
        yield batch_x, batch_base_seq, batch_y

# Computes global metrics such as loss, precision, recall, F1 score, and accuracy from per-group results.
# Returns metrics as formatted text and optionally as a dictionary.
def get_stats(metrics_dict, dtype, return_dict=False):
    ...
    if return_dict:
        return '\n'.join([loss_str, acc_str, prec_str, rec_str, f1_str]), {
            'loss': total_loss,
            'accuracy': total_acc,
...
# Updates the cumulative metrics dictionary using the confusion matrix from a batch.
